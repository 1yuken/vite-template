...
- `src/pages/` — страницы сайта; URL в dev и в сборке: `/имя.html` → файл `имя.pug` (перезапуск dev после новой страницы не нужен).
- `src/index.yaml` — заголовок и порядок страниц в dev-хабе; все `src/pages/*.pug` подхватываются автоматически (можно не дублировать в YAML).
